...
export default function EditPropertyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchProperty = async () => {
    try {
      const data = await api.get<{ property: any }>(`/api/admin/properties/${id}`)
      if (!data) throw new Error("Gayrimenkul yüklenemedi")
      ...
      if (property.type === "RESIDENTIAL" && property.residentialDetails) {
        setResidentialData(property.residentialDetails)
      } else if (property.type === "COMMERCIAL" && property.commercialDetails) {
        setCommercialData(property.commercialDetails)
      } else if (property.type === "LAND" && property.landDetails) {
        setLandData(property.landDetails)
      }
      ...
      setError(err.message || "Gayrimenkul yüklenirken hata oluştu")
    } finally {
      setFetching(false)
    }
  }
  ...
  const handleLandChange = (field: string, value: any) => {
    setLandData((prev: any) => ({ ...prev, [field]: value }))
  }
  ...
  const uploadNewImages = async (): Promise<any[]> => {
    ...
    for (let i = 0; i < newImages.length; i++) {
      ...
      uploadedImages.push({
        ...
        alt: formData.title || `Gayrimenkul görseli`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // id ve propertyId alanlarını çıkarıyoruz çünkü bunlar create işleminde kullanılmamalı
      const allImages = [
      ...
      const propertyData: any = {
        ...
        featuredOrder: formData.featured ? formData.featuredOrder : null,
        slug: formData.slug,
        metaTitle: formData.metaTitle || formData.title,
        metaDescription: formData.metaDescription || formData.description.substring(0, 160),
      }

      // Tip-spesifik detaylardan id ve propertyId alanlarını çıkar
      const cleanResidentialData = residentialData ? Object.fromEntries(
        Object.entries(residentialData).filter(([key]) => key !== 'id' && key !== 'propertyId')
      ) : undefined

      const cleanCommercialData = commercialData ? Object.fromEntries(
        Object.entries(commercialData).filter(([key]) => key !== 'id' && key !== 'propertyId')
      ) : undefined

      const cleanLandData = landData ? Object.fromEntries(
        Object.entries(landData).filter(([key]) => key !== 'id' && key !== 'propertyId')
      ) : undefined

      const result = await api.put<{ property: any }>(`/api/admin/properties/${id}`, {
        ...propertyData,
        images: allImages,
        ...
        landDetails: formData.type === "LAND" ? cleanLandData : undefined,
      })

      if (!result) {
        throw new Error(api.error?.message || "Gayrimenkul güncellenemedi")
      }

      toast.success("Gayrimenkul başarıyla güncellendi")
  ...
  const confirmDeleteProperty = async () => {
    setDeleting(true)
    try {
      const result = await api.del(`/api/admin/properties/${id}`)

      if (!result) throw new Error("Gayrimenkul silinemedi")
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-8">
      <div className="mb-4 sm:mb-6 lg:mb-8">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-text-heading">Gayrimenkulu Düzenle</h1>
        <p className="text-text-secondary mt-1 sm:mt-2 text-sm sm:text-base">Gayrimenkul bilgilerini güncelleyin</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6 sm:space-y-8">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <Input
                ...
                required
                placeholder="Örn: 3+1 Satılık Daire Beşiktaş"
              />
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Gayrimenkul Tipi *</label>
                <div className="text-sm text-gray-500">
                  {PROPERTY_TYPE_MAIN_OPTIONS[formData.type as keyof typeof PROPERTY_TYPE_MAIN_OPTIONS]}
                </div>
              </div>
            </div>
            ...
            {formData.type === "LAND" && (
              <div className="space-y-4 sm:space-y-6 border-t border-border pt-6 sm:pt-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                  <Select
                    label={LAND_LABELS.propertyType}
                    value={landData.propertyType || ""}
                    onChange={(value) => handleLandChange('propertyType', value)}
                    options={Object.entries(PROPERTY_TYPE_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                  <Select
                    label={LAND_LABELS.zoningStatus}
                    value={landData.zoningStatus || ""}
                    onChange={(value) => handleLandChange('zoningStatus', value)}
                    options={Object.entries(ZONING_STATUS_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                  <Input
                    label={LAND_LABELS.area}
                    type="number"
                    value={landData.area || ""}
                    onChange={(value) => handleLandChange('area', value ? parseFloat(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.pricePerSqm}
                    type="number"
                    value={landData.pricePerSqm || ""}
                    onChange={(value) => handleLandChange('pricePerSqm', value ? parseFloat(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.islandNumber}
                    type="number"
                    value={landData.islandNumber || ""}
                    onChange={(value) => handleLandChange('islandNumber', value ? parseInt(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.parcelNumber}
                    type="number"
                    value={landData.parcelNumber || ""}
                    onChange={(value) => handleLandChange('parcelNumber', value ? parseInt(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.sheetNumber}
                    type="number"
                    value={landData.sheetNumber || ""}
                    onChange={(value) => handleLandChange('sheetNumber', value ? parseInt(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.floorAreaRatio}
                    type="number"
                    step="0.01"
                    value={landData.floorAreaRatio || ""}
                    onChange={(value) => handleLandChange('floorAreaRatio', value ? parseFloat(value) : null)}
                  />
                  <Input
                    label={LAND_LABELS.heightLimit}
                    type="number"
                    step="0.01"
                    value={landData.heightLimit || ""}
                    onChange={(value) => handleLandChange('heightLimit', value ? parseFloat(value) : null)}
                  />
                  <Select
                    label={LAND_LABELS.mortgageEligible}
                    value={landData.mortgageEligible || ""}
                    onChange={(value) => handleLandChange('mortgageEligible', value)}
                    options={Object.entries(MORTGAGE_ELIGIBLE_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                  <Select
                    label={LAND_LABELS.deedStatus}
                    value={landData.deedStatus || ""}
                    onChange={(value) => handleLandChange('deedStatus', value)}
                    options={Object.entries(DEED_STATUS_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                  <Select
                    label={LAND_LABELS.listedBy}
                    value={landData.listedBy || ""}
                    onChange={(value) => handleLandChange('listedBy', value)}
                    options={Object.entries(LISTED_BY_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                  <Select
                    label={LAND_LABELS.exchange}
                    value={landData.exchange || ""}
                    onChange={(value) => handleLandChange('exchange', value)}
                    options={Object.entries(EXCHANGE_OPTIONS).map(([key, label]) => ({ value: key, label }))}
                  />
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 pt-4 sm:pt-6 border-t border-border">
              <Button type="button" variant="delete" onClick={() => setShowDeletePropertyDialog(true)} disabled={deleting} className="order-2 sm:order-1">
                {deleting ? "Siliniyor..." : "Gayrimenkulu Sil"}
              </Button>
      ...
      {/* Gayrimenkul Silme Onay Dialogu */}
      <ConfirmModal
        isOpen={showDeletePropertyDialog}
        title="Gayrimenkulu Sil"
        message="Bu gayrimenkulu silmek istediğinizden emin misiniz? Tüm görseller ve veriler silinecektir."
        confirmText="Gayrimenkulu Sil"
        cancelText="İptal"
        onConfirm={confirmDeleteProperty}
        onCancel={() => setShowDeletePropertyDialog(false)}
        isLoading={deleting}
      />
    </div>
  )
