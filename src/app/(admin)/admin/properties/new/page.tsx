...
export default function NewPropertyPage() {
  ...
  const uploadImages = async (): Promise<any[]> => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      formDataUpload.append("file", images[i].file)

      const data = await api.upload<{ url: string; publicId: string }>("/api/admin/upload", formDataUpload)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.type === "RESIDENTIAL") {
        propertyData.residentialDetails = residentialData
      } else if (formData.type === "COMMERCIAL") {
        propertyData.commercialDetails = commercialData
      } else if (formData.type === "LAND") {
        propertyData.landDetails = landData
      }

      const result = await api.post<{ property: any }>("/api/admin/properties", propertyData)

      if (!result) {
        throw new Error(api.error?.message || "Gayrimenkul oluşturulamadı")
      }

      toast.success("Gayrimenkul başarıyla oluşturuldu")
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6 lg:py-8">
      <div className="mb-4 sm:mb-6 lg:mb-8">
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-text-heading">Yeni Gayrimenkul Ekle</h1>
        <p className="text-text-secondary mt-1 sm:mt-2 text-sm sm:text-base">Satılık veya kiralık gayrimenkulünüzü ekleyin</p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6 sm:space-y-8">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <Select
                ...
                required
                options={Object.entries(PROPERTY_TYPE_MAIN_OPTIONS).map(([key, label]) => ({ value: key, label }))}
              />
            </div>
            ...
            <div className="space-y-4 sm:space-y-6 border-t border-border pt-6 sm:pt-8">
              ...
              <FileUpload
                ...
                helperText="İlk yüklenen görsel otomatik olarak kapak görseli olur. Değiştirebilirsiniz."
              />
            ...
            <div className="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-border">
              ...
              <Button type="submit" variant="save" disabled={loading || uploadingImages} loading={loading || uploadingImages}>
                {uploadingImages ? "Görseller Yükleniyor..." : loading ? "Kaydediliyor..." : "Gayrimenkulü Kaydet"}
              </Button>
            </div>
          </form>
        </div>
